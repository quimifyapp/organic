...
public:
	...
	Substituent(Id function, unsigned short bonds) :
		function(function), bonds(bonds), carbons(0), iso(false)
	{
	}

	Substituent(unsigned short carbons, bool iso) :
		function(Id::radical), bonds(1), carbons(carbons), iso(iso)
	{
	}

	bool equals(Substituent s)
	{
		return function == s.function && bonds == s.bonds &&
			carbons == s.carbons && iso == s.iso;
	}
...
protected:
	...
	vector<unsigned short> listBonds(unsigned short bonds)
	{
		...
		bonds -= 1;
		for (unsigned short i = 0; i < carbons.size(); i++)
			if (carbons[i].getFreeBonds() == bonds)
...
private:
	...
	void correct()
	{
				...
				if (largest_radical.getCarbons())
				{
					...
					if (largest_radical.getCarbons() - largest_radical.getIso() > i)
					{
						...
						for (unsigned short k = 0; k < i && change_left; k++)
						{
							...
							// basic_chain[i]
							for (unsigned short j = 0; j < carbons[k].getAllSubs().size(); j++)
								// Los sustituyentes de esos carbonos
								if (carbons[k].getAllSubs()[j].getFunction() != Id::hydrogen &&
									!(k == carbons.size() - 2 &&
										carbons[k].getAllSubs()[j].equals(methyl) &&
					...
					if (change_left || change_right)
					{
						...
						Carbon CH2(2);
						CH2.addSubstituent(hydrogen);
						CH2.addSubstituent(hydrogen);
						// Eslabón CH2, constante útil
						Carbon CH3(1);
						CH3.addSubstituent(hydrogen);
						CH3.addSubstituent(hydrogen);
						CH3.addSubstituent(hydrogen);
						...
						{
							unsigned short index = apendix.size() - 2;
							apendix[index].deleteSubstituentWithBonds(hydrogen);
							apendix[index].addSubstituent(methyl);
							apendix[++index].deleteBond();
							apendix[index].addSubstituent(hydrogen);
						}
						else apendix.push_back(CH3);
						...
						{
							iso = carbons.size() > 2 &&
								carbons[carbons.size() - 2].thereIs(methyl);
						...
						{ // Left
							iso = carbons.size() > 2 && carbons[1].thereIs(methyl);
		...
		if (functions.size())
		{
			...
			{
				carbons[0].deleteSubstituentWithBonds(list.find(Id::ketone)->second);
				carbons[0].deleteSubstituentWithBonds(list.find(Id::alcohol)->second);
				carbons[0].addSubstituent(list.find(Id::acid)->second);
				listUniqueFunctions();
			}
			if (carbons[carbons.size() - 1].thereIs(Id::ketone) &&
				carbons[carbons.size() - 1].thereIs(Id::alcohol))
			{
				carbons[carbons.size() - 1].deleteSubstituentWithBonds(
					list.find(Id::ketone)->second);
				carbons[carbons.size() - 1].deleteSubstituentWithBonds(
					list.find(Id::alcohol)->second);
				carbons[carbons.size() - 1].addSubstituent(
					list.find(Id::acid)->second);
				listUniqueFunctions();
			}
			...
			if (functions[0] != Id::amide)
			{
				...
				{
					carbons.erase(carbons.begin());
					carbons[0].deleteBond();
					carbons[0].addSubstituent(carbamoyl);
					listUniqueFunctions();
				}
				else if (carbons[carbons.size() - 1].thereIs(Id::amide))
				{
					carbons.pop_back();
					carbons[carbons.size() - 1].deleteBond();
					carbons[carbons.size() - 1].addSubstituent(carbamoyl);
					listUniqueFunctions();
				}
			}
			...
			if (functions[0] != Id::nitrile)
			{
				...
				if (carbons[0].thereIs(Id::nitrile))
				{
					carbons.erase(carbons.begin());
					carbons[0].deleteBond();
					carbons[0].addSubstituent(cyanide);
					listUniqueFunctions();
				}
				else if (carbons[carbons.size() - 1].thereIs(Id::nitrile))
				{
					carbons.pop_back();
					carbons[carbons.size() - 1].deleteBond();
					carbons[carbons.size() - 1].addSubstituent(cyanide);
					listUniqueFunctions();
				}
			}
			...
			if (functions[0] >= Id::aldehyde)
			{
				...
				{
					carbons[0].deleteSubstituentWithBonds(ketone);
					carbons[0].deleteSubstituentWithBonds(hydrogen);
					carbons[0].addSubstituent(aldehyde);
					listUniqueFunctions();
				}
				if (carbons[carbons.size() - 1].thereIs(Id::ketone)
					&& carbons[carbons.size() - 1].thereIs(Id::hydrogen))
				{
					carbons[carbons.size() - 1].deleteSubstituentWithBonds(ketone);
					carbons[carbons.size() - 1].deleteSubstituentWithBonds(hydrogen);
					carbons[carbons.size() - 1].addSubstituent(aldehyde);
					listUniqueFunctions();
				}
			}
			...
			if (functions[0] != Id::aldehyde)
			{
				...
				{
					carbons[carbons.size() - 1].deleteSubstituent(aldehyde);
					carbons[carbons.size() - 1].addSubstituent(ketone);
					carbons[carbons.size() - 1].addSubstituent(hydrogen);
					listUniqueFunctions();
				}
				else if (carbons[0].thereIs(Id::aldehyde))
				{
					carbons[0].deleteSubstituent(aldehyde);
					carbons[0].addSubstituent(ketone);
					carbons[0].addSubstituent(hydrogen);
					listUniqueFunctions();
				}
			}
		}
			...
			else if (functions[i] == Id::radical)
			{
				...
				for (unsigned short j = 0; j < normal_radicals.size(); j++)
				{
					unsigned short result =
						compareAlphabetically(normal_radicals[j], reversed_radicals[j]);
					if (result == 1)
						carbons = reversed.carbons;
					if (result != 2)
						break;
	...
	bool isRedundant(Id function, vector<unsigned short> positions)
	{
		...
		else if (carbons.size() == 2)
		{
			...
			if (functions_temp.size() == 2)
			{
				...
				unsigned short sum = sbts::list.find(
					functions_temp[0])->second.getBonds() +
		...
		else if (carbons.size() == 3)
		{
			if (function == Id::ketone && functions[0] >= Id::aldehyde)
				...
				return true;
			if (functions.size() == 1 &&
				(function == Id::alkene || function == Id::alkyne))
	...
	Locator prefixFor(Id function) {
		const static map<Id, string> prefixes{
			...
			{Id::amine, "amino"},{Id::nitro, "nitro"},
			{Id::bromine, "bromo"},{Id::chlorine, "cloro"},
			{Id::fluorine, "fluoro"},{Id::iodine, "yodo"}
		};

		vector<unsigned short> positions = listPositionsOf(function);

		if (isRedundant(function, positions))
			return Locator("", quantifier(positions.size()), prefixes.find(function)->second);
		if (isHalogen(function) && getUniqueSubstituents().size() == 1)
			return Locator("", "per", prefixes.find(function)->second);

		return locatorFor(positions, prefixes.find(function)->second);
	}

	string sufixFor(Id function)
	{
		const static map<Id, string> sufixes{
			{Id::acid, "oico"},{Id::amide, "amida"},{Id::nitrile, "nitrilo"},
			{Id::aldehyde, "al"},{Id::ketone, "ona"},{Id::alcohol, "ol"},
			{Id::amine, "amina"}
		};
		...
		{
			return quantifier(positions.size()) + sufixes.find(function)->second;
		}
		return locatorFor(positions, sufixes.find(function)->second).toString();
	}
...
public:
	...
	string getName()
	{
		...
		if (carbons.size() == 1 && functions.size() == 1 &&
			functions[0] == Id::ketone && !thereIs(Id::hydrogen))
		...
		if (functions.size() && functions[0] != Id::nitro &&
			functions[0] != Id::radical && functions[0] != Id::alkene &&
			functions[0] != Id::alkyne && !isHalogen(functions[0]))
		{
	...
	string getFormula()
	{
		// CH(HO) -> CH2O
		// CHO(OH) -> CH2O2
		string s = carbons[0].toString();
		for (unsigned short i = 1; i < carbons.size(); i++)
		{
			s += ((carbons[i - 1].getFreeBonds())
				? ((carbons[i - 1].getFreeBonds() == 1)
					? "="
					: "=-") // Actually '≡'
				: "-") + carbons[i].toString();
		}
	...
	{
		vector<Id> result;
		switch (carbons.back().getFreeBonds())
		{
		case 4:
		case 3:
	...
	unsigned short getFreeBonds()
	{
		return carbons.back().getFreeBonds();
	}
	...
	void nextCarbon()
	{
		if (carbons.size())
		{
			...
			carbons.push_back(Carbon(carbons.back().getFreeBonds() + 1));
		}
...
class Cyclic : protected Organic {
	...
	-- ->  ~CH(A)-CH(B)-CH2-CH2-CH2-CH2~ | p-[A][B]benceno
	*/
protected:
	void listUniqueFunctions()
	{
		...
		for (unsigned short i = 0; i < carbons.size(); i++)
		{
			...
			for (unsigned short j = 0; j < carbons[i].getAllSubs().size(); j++)
				if (find(functions.begin(), functions.end(),
					carbons[i].getAllSubs()[j].getFunction()) == functions.end() &&
					carbons[i].getAllSubs()[j].getFunction() != Id::hydrogen)
				{
...
public:
	Aromatic()
	{
		// Aromatic < Cyclic < Organic already starts with a Carbon
		nextCarbon(); // 2nd
		nextCarbon(); // 3th
		nextCarbon(); // 4th
		nextCarbon(); // 5th
		nextCarbon(); // 6th
		carbons.back().bondCarbon(); // Bonded to the first one
	}
	...
	{
		return carbons[index].getFreeBonds();
	}
	...
	Locator prefixFor(Substituent sub1, Substituent sub2, unsigned short distance)
	{
		Locator locator;
	...
	string getName()
	{
		...
		if (listPositionsOf(Id::hydrogen).size() == 4)
		{
			...
			for (unsigned short i = 1; i < subs.size(); i++)
				...
				{
					pre = prefixFor(subs[0], subs[i], i).toString();
					break;
				}
		}
		else
...
void randomTest()
{
	...
	{
		Basic compound;
		unsigned short carbons = getRandomNumber(1, MAX_CARB);
		for (unsigned short i = 0; i < carbons; i++)
		{
			for (vector<Id> available = compound.availableSubstituents();
				compound.getFreeBonds() > (i != (carbons - 1));
				available = compound.availableSubstituents())
			{
				unsigned short type = getRandomNumber(0, available.size() - 1);
				if (available[type] == Id::radical)
				{
					if (getRandomNumber(0, 1))
						compound.addSubstituent(Substituent(getRandomNumber(1, MAX_RAD), false));
					else if (compound.getFreeBonds() > (1 + (i != (carbons - 1))) && getRandomNumber(0, 1))
					{
						compound.addSubstituent(sbts::methyl);
						compound.addSubstituent(sbts::methyl);
					}
					else compound.addSubstituent(Substituent(getRandomNumber(3, MAX_RAD), true));
				}
				else if (available[type] == Id::halogen)
				{
					switch (getRandomNumber(0, 3))
					{
					case 0:
						compound.addSubstituent(sbts::bromine);
						break;
					case 1:
						compound.addSubstituent(sbts::chlorine);
						break;
					case 2:
						compound.addSubstituent(sbts::fluorine);
						break;
					case 3:
						compound.addSubstituent(sbts::iodine);
						break;
					}
				}

				else if (compound.getFreeBonds() > sbts::list.find(available[type])->second.getBonds())
				{
					compound.addSubstituent(sbts::list.find(available[type])->second);
				}

				if (i < (carbons - 1) && !getRandomNumber(0, 2))
					break;
			}
			if (i < (carbons - 1)) compound.nextCarbon();
		}

		cout << endl;
		cout << compound.getFormula() << endl;
		cout << " ---> " << compound.getName() << endl;
		cout << " ---> " << compound.getFormula() << endl;
...
int main()
{
	const map<Id, string> texts{
		{Id::acid, "-=OOH"},{Id::carboxyl, "-COOH"},{Id::amide, "-=ONH2"},
		{Id::carbamoyl, "-CONH2"},{Id::nitrile, "-=N"},{Id::cyanide, "-CN"},
		{Id::aldehyde, "-=OH"},{Id::ketone, "=O"},{Id::alcohol, "-OH"},
		{Id::amine, "-NH2"},{Id::nitro, "-NO2"},{Id::halogen, "-X"},
		{Id::radical, "-CH2-CH2..."},{Id::hydrogen, "-H"}
	};
	...
	while (AROMATIC)
	{
		...
		for (unsigned short c = 0; c < 6; c++)
		{
			...
			if (!first) cout << SEPARADOR << endl << " 0) " << "-C-" << endl;
			else first = false;

			cout << SEPARADOR << endl;
			for (unsigned short i = 0; i < available.size(); i++)
				cout << ' ' << i + 1 << ") " << texts.find(available[i])->second <<
				endl << SEPARADOR << endl;
			...
			for (unsigned short i = 0; i < available.size(); i++)
				if (input == i)
				{
					...
					{
						cout << SEPARADOR << endl << " 1) -CH2-...-CH3" <<
							endl << SEPARADOR << endl << "                  CH3" <<
							endl << "                 /" << endl << " 2) -CH2-...-CH2" <<
							endl << "                 \\" << endl << "                  CH3" <<
							endl << SEPARADOR << endl << " Seleccionar: ";
						cin >> input;
						unsigned short carbons;
						if (input == 1)
						{
					...
					else if (available[i] == Id::halogen)
					{
						...
						{
							cout << SEPARADOR << endl;
							cout << " 1) " << "-Br" << endl << SEPARADOR << endl;
							cout << " 2) " << "-Cl" << endl << SEPARADOR << endl;
							cout << " 3) " << "-F" << endl << SEPARADOR << endl;
							cout << " 4) " << "-I" << endl << SEPARADOR << endl;
							cout << " Seleccionar: ";
							cin >> input;
						} while (input > 4);
		...
		cout << " ---> " << aromatic.getFormula() << endl << endl;
		system("pause");
	}
	...
	while (BASIC)
	{
		...
		*/
		// Cadenas con bucle
		bool first = true;
		for (vector<Id> available = basic_chain.availableSubstituents(); available.size();
			available = basic_chain.availableSubstituents())
		{
			cout << endl << " ---> " << basic_chain.getFormula() << endl;

			if (!first)
				cout << SEPARADOR << endl << " 0) " << "-C-" << endl;
			else first = false;
					...
					if (input == i)
					{
						if (available[i] == Id::radical)
						{
							...
							else if (input == 2)
							{
								...
								basic_chain.addSubstituent(Substituent(carbons, true));
							}
						}
						else if (available[i] == Id::halogen)
						{
		...
		cout << " ---> " << basic_chain.getFormula() << endl << endl;
		system("pause");
	}
