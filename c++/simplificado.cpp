...
// Es como main.cpp pero sin funciones para nombrar compuestos.
...
protected:
	...
	string concatenate(vector<string> vector)
	{
		string result;
		for (unsigned short i = 0; i < vector.size(); i++)
			result += "-" + vector[i];
		...
		else if (carbons.size() == 2)
		{
			...
			if (functions_temp.size() == 2)
			{
				...
				unsigned short sum = sbts::list.find(
					functions_temp[0])->second.getBonds() + sbts::list.find(functions_temp[1])->second.getBonds();
				if (sum > 3)
